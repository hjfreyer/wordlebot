 ...
 *
 * Continuation after the second guess is greedy over surviving candidates. That
 * approximation is far weaker here than for whole-game play -- by move three the
 * surviving sets are tiny, and with one or two candidates left there is nothing
 * for a probe to do.
 */
...
function scoreSecond(S, W, totalWeight, second, decisions) {
  let total = 0;
  const byPattern = new Map();
  for (let i = 0; i < S.length; i++) {
  ...
  for (const b of byPattern.values()) {
    total += turnsFrom(b.idx, Float64Array.from(b.w), 3, decisions);
  }
...
function buildOpener(word) {
  ...
  for (const [pattern, S] of [...states.entries()].sort((a, b) => b[1].length - a[1].length)) {
    ...
    out.push({
      ...
      best: scored.slice(0, TOP_N).map(([i, t]) => [words.list[i], Math.round(t * 1000) / 1000]),
    });
  }
